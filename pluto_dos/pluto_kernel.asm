...
BSOUT   .proc
        ...
        BRA  COUT     ; then done BSOUT subroutine, RETURN
        .pend
...
COUT3:  .proc
        JSR  COUT     ;(Send byte 3 times)
        .pend

COUT2:  .proc
        JSR  COUT     ;(Send byte 2 times)
        .pend
...
SENGBR3:
        ...
        BRA  SENGBR2  ;LOOP back to SENGBR2
SENGBR1:
...
STBR2:
        ...
        BRA  STLOOP   ;LOOP back to STLOOP
STBR3:
...
RET     RTS           ;Done LISTER command, RETURN. RET label is defined for [BREAK] button
        .pend
...
SKIP1:
        ...
        JSR  COUT3
        BRA  REENTR   ;LOOP back to REENTER
;Process directives
DIRECTV:
...
BYOK:
        ...
        BRA  BYLOOP   ;LOOP back to BYLOOP
        .pend
...
MORDIS:
        ...
        BNE  LSTNEW
        JSR  DISLINE  ;List (disassemble) one instruction
        BRA  MORLOOP  ;LOOP back to MORLOOP
...
WDOK:
        ...
        STA  (INDEX,X)
        JSR  INCNDX   ;Increment working address pointer
        BRA  WDLOOP   ;LOOP back to WDLOOP
;
;
;
...
H2LOOP:
        ...
        BRA  H2LOOP   ;LOOP back to H2LOOP
H2OK:
...
H4LOOP:
        ...
        BNE  H4OK
        JSR  ERRBS    ; ELSE, send [BELL],[BACKSPACE] to terminal
        BRA  H4LOOP   ;LOOP back to H4LOOP
H4OK:
...
NOTZ2S:
        JSR  BEEP     ;  send [BELL] to terminal
        BRA  BITSEL   ;LOOP back to BITSEL
Z2SOK:
...
TWOBYT:
        ...
        JSR  MSPC
        BRA  PRMNEM   ;Display mnemonic then done TWOBYT subroutine, RETURN
;
...
TRIBYT:
        ...
        JSR  SPC2     ;Send 2 [SPACE] to terminal
        BRA  PRMNEM   ;Display mnemonic then done TRIBYT subroutine, RETURN
;
...
SUBTRACT:
        ...
        LDA  INDEXH
        SBC  #$00
        BRA  RELLOOP  ;LOOP back to LELLOOP
;
;
;
;Assembler handlers:
...
STOR2:
        ...
        BRA  STOR     ;GOTO STOR (write operand byte to working memory)
CSTOR3:
        JSR  COUT     ;Send character in ACCUMULATOR to terminal
LSTOR3:
        LDA  OCTAB,Y  ;Read indexed opcode from opcode table
...
LDACC:
        ...
        BRA  LSTOR2   ;Write 2 byte instruction to working memory then GOTO NEWLIN
NOTIMED:
...
STAIN:
        ...
        BRA  LSTOR3   ;   ELSE, write 3 byte instruction to working memory then GOTO NEWLIN
;
BFIXBR1:
